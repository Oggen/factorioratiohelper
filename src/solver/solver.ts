...
export default class RecipeSolver {
    ...
    /**
     * 
     * @param allSteps All of the steps involved in this problem. 
     *      can include steps not necissarily involved in the problem
     * @param desiredOutputs The outputs desired, in both name and amount
     * @param inputRatios All of the inputs that will be accepted; with
     *      an accompanying number that indcates the relative cost of the input
     * @param slackOutputs The possible extra output resources, 
     *      and the prefferential rate that they should be output into.
     *      Similar to inputRations, except for outputs. If left out, then it is assumed
     *      all non-input and non-output resources are slack outputs with a weight of 1,000,000,000
     */
    constructor(
        allSteps: Step[],
        desiredOutputs: ResourceCount[],
        inputRatios: ResourceCount[],
        slackOutputs?: ResourceCount[],
        log: (str: string) => void = a => undefined ) {
    ...
    private addResourcePipeWithWeight(
        resourceName: string, 
