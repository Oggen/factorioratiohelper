...
const reduxApp : Reducer<RootState> = function(state = initialState, action : RootAction): RootState {
    switch (action.type) {
        ...
        case actionTypes.CREATE_STEP_INPUT:
            return update(state, {steps: {[action.stepIndex]: {inputs: {$push: [new ResourceCount]}}}});
        ...
        case actionTypes.CREATE_STEP_OUTPUT:
            return update(state, {steps: {[action.stepIndex]: {outputs: {$push: [new ResourceCount]}}}});
        ...
        case actionTypes.ADD_RESOURCE_COUNT:
            return update(state, {[action.resourceGroup]: {$unshift: [
                new ResourceCount
            ]}});
        ...
        case actionTypes.UPDATE_RESOURCE:
            return update(state, {[action.resourceGroup]: {[action.resourceIndex]: {resource: {$set: action.resource}}}});
        case actionTypes.UPDATE_COUNT:
            return update(state, {[action.resourceGroup]: {[action.resourceIndex]: {count: {$set: action.count}}}});
